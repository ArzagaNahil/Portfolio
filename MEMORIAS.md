...
## Sesión 36 - Las cuatro galerías de Fotografía con la tarjeta de Diseño, y Foto artística

**Lo que pidió el usuario**: que las galerías de fotografía tuvieran las mismas dimensiones que las de diseño y las tarjetas se movieran a la misma velocidad en todos los carruseles; que las imágenes de Retrato y Paisaje, que estaban como marcadores de posición, se pusieran de verdad; y queRetrato, Paisaje y Producto tuvieran una hermana más llamada **Foto artística**, con las mismas características.

**Los originales, medidos antes de tocar nada**: `retrato` 13 ficheros (12,83 MB), `paisaje` 39 (233,91 MB), `Foto_artistica` 27 (145,60 MB) y `producto` 43 (114,10 MB). En la raíz de la carpeta hay tres ficheros sueltos (`alexis1.jpg`, `foto alex hv2.jpg`, `foto alex hv3.jpg`) que el usuario confirmó que no son del portafolio, así que no se tocan. Dos de las 39 de Paisaje y cuatro de las 27 de Foto artística son de **12240×16320**, unos 200 Mpx cada una: Pillow lanza `DecompressionBombError` en el `Image.open` de por sí, así que hace falta `Image.MAX_IMAGE_PIXELS = None` y `Image.draft()` para no meter 600 MB de RAM por foto. Seis fotos en total.

**Un solo criterio para las cuatro galerías**: miniatura WebP con caja 2000×506 y calidad 80, grande WebP con caja 2000×2000 y calidad 82, `method=6`, **proporción natural y sin recorte**. De 506,44 MB de origen a **36,14 MB**, el 7,1 %. Producto se regenera con el mismo criterio, de 9,67 MB a 8,82 MB, y así las cuatro hermanas pesan igual por foto. Los originales no se tocan nunca.

**La tarjeta**: las de fotografía eran `.gallery__item` con `aspect-ratio: 4 / 5` y `object-fit: cover`, o sea que **recortaban** las fotos. Las de diseño son `.gallery__card`, con `height: clamp(250px, 41vh, 506px)` y `width: auto`, que no recorte nada porque el ancho sale de la proporción natural de cada imagen. Las cuatro de fotografía pasan a `.gallery__card`. La diferencia no es solo estética: al fijar la altura y no el ancho, el ancho del track deja de depender del ancho de la ventana.

**La velocidad**: `PX_POR_SEGUNDO = 275` ya era única para las ocho galerías, pero la duración se recalculaba solo si cambiaba `innerHeight`. Como las tarjetas de Desarrollo son `width: clamp(220px, 22vw, 340px)`, redimensionar **en horizontal** las desincronizaba. Ahora el `resize` mira el ancho y el alto, y añadido un remedido en `window.load`, porque la primera medición sale al construir el DOM y, si el navegador aún no ha resuelto el ancho de cada `<img>`, el track mide menos y la velocidad queda más alta.
...
**Error propio**: la clonación de `gallery-photo-4.html` a partir de la de Producto dejó dos cosas mal. El `og:title` seguía diciendo "Producto — Arzaga Nahil", porque el reemplazo buscía el texto encerrado en `>` y el `og:title` lo lleva en `content="..."`. Y el `</p>` de la descripción desapareció, porque el reemplazo de la descripción se llevó hasta `</p>` y no lo reponía. Un clon de HTML por `replace` arrastra lo que no es de la galería; hay que repasar la cabecera y el cuerpo uno por uno.

**Error propio, el que más tiempo costó**: la edición de `js/i18n.js` se hizo primero por posición de cadena, buscando `
    en: {` y `
};`, y salió mal: el trozo que se tomó por el bloque `en` era en realidad **la cola del fichero**, el código que aplica las traducciones. Un idioma habría salido con 2.499 caracteres de traducciones y el otro con 17.667. Se rehíz **por líneas**: se localizan las líneas `    es: {`, `    },`, `    en: {`, `    },` y se trabaja sobre la lista de líneas de cada cuerpo, de forma que una sustitución no puede saltar de un idioma al otro. Es el mismo error de la sesión 34 con otra forma: **editando las dos mitades de un fichero bilingþe por separado**.

**Error propio, y silencioso**: varios patrones de búsqueda estaban escritos con `
` y se ejecutaron contra ficheros que usan CRLF. El del `resize` de `index.js` no encontró su ancla y el script **salió con código 0** en vez de fallar. Se detectó porque el bloque no cambió y la comprobación posterior dijo que `ultimoAncho` no estaba. Menos mal: la comprobación posterior es la que detectó el fallo, no el script.

**Error propio, del verificador, cuatro veces seguidas**: la primera tanda de comprobaciones dio 5 fallos que no eran del sitio.
- Contaba que el número de `</button>` fuera el de tarjetas **má 3**, suponiendo que solo había los tres del visor, y se olvidaba del botón del menú. Se sustituyó por un validador de anidamiento con `html.parser`, que adás comprueba que las 16 páginas están bien estructuradas.
- Comparaba una lista de cadenas contra una lista de tuplas al cotejar los nombres de fichero del manifiesto con el HTML, así que nunca podían coincidir.
- Contaba 117 ficheros de origen cuando en las cuatro carpetas hay **122**: los 5 de Producto que el usuario dejó fuera. El 117 es el que entra, no el que hay.
- El recuento de claves de i18n salía **363 en ES y 352 en EN**, y parecía una asimetría grave. No lo era: el regex de lectura solo aceptaba valores entre comillas **simples**, y en inglés hay once valores con comillas **dobles** porque llevan apóstrofo ("Hi, I'm", "Let's work together"). Los dos idiomas tienen las mismas 363. Es la cuarta vez que un verificador da un OK o un FAIL que no se corresponde con el código: **el verificador también se revisa**.

**El `width="400" height="300"` de las tiles era inerte**: `.tile__fig::before` fija la caja con `padding-top: 136.36%` y el `.tile__img` va en `position: absolute` con `object-fit: cover`, así que la medida del atributo no cambiaba nada, y además decía 4:3 cuando ninguna foto es 4:3. Se pone la medida real de cada fichero. Y al medir se vio que las tiles **no tienen el mismo tamaño**: `retrato` es 800×1090 y las demás 1296×1776. Da igual mientras el recorte sea el mismo, pero la de Foto artística se genera a 1296×1776, que es la mayor.

**i18n**: de 289 a 363 claves por idioma. Se añaden 9 nuevas por idioma (`g_desc_photo_artistica`, `pt_galphoto4`, `tile_artistica`, `alt_photo_4`, `lb_gallery_retrato`, `lb_gallery_paisaje`, `lb_gallery_artistica`, `meta_title_galphoto4`, `meta_desc_galphoto4`), los 117 `galphoto*_img_*` y se borran 8 en cada idioma: `galphoto1_card1`, `galphoto2_card1` y los seis `galphoto1_demo_*` y `galphoto2_demo_*`, que eran de las tarjetas de ejemplo y sus enlaces a demo.
